...
pub fn action_permission(action_id: i32) -> Result<(Resource, Action)> {
    ...
    match action_id {
        ...
        _ => {
            ...
            Err(Error::Internal("unexpected error".into()))
        }
    }
}
...
pub fn action_organisation(
    ...
    let school_from_field1 = |payload: &[u8]| -> Result<Organisation> {
        ...
        let id: Id = p.school.parse().map_err(|e| {
    ...
    match action_id {
        ...
        UPDATE_SCHOOL | DELETE_SCHOOL => {
            let p: IdField = decode(payload)?;
            let id: Id = p.id.parse().map_err(|e| {
            ...
            match p.school {
                Some(s) if !s.is_empty() => {
                    let id: Id = s.parse().map_err(|_| {
                        ...
                        Error::Internal("unexpected error".into())
                    })?;
                    Ok(Organisation::School(id))
                }
                _ => Ok(Organisation::System),
...
fn fetch_school(conn: &mut Conn, id: &str) -> Result<SchoolRow> {
    sql_query("SELECT id, name, motto, phone, email, county, domain, established, status, created, updated FROM schools WHERE id = ?")
        .bind::<Text, _>(id)
        .load::<SchoolRow>(conn)
        .map_err(|e| { tracing::error!("fetch_school failed: {e}"); Error::internal(e) })?
        .into_iter()
        .next()
        .ok_or_else(|| { tracing::error!("school not found: {id}"); Error::Internal("school not found: {id}".into()) })
}
...
fn fetch_owner(conn: &mut Conn, school: &str, user: &str) -> Result<OwnerRow> {
    sql_query("SELECT school, user, created FROM owners WHERE school = ? AND user = ?")
        ...
        .ok_or_else(|| {
            ...
            Error::Internal("owner not found: {school}|{user}".into())
        })
}
...
fn fetch_plan(conn: &mut Conn, id: &str) -> Result<PlanRow> {
    sql_query("SELECT id, name, description, amount, levels, status, features, created, updated FROM plans WHERE id = ?")
        .bind::<Text, _>(id)
        .load::<PlanRow>(conn)
        .map_err(|e| { tracing::error!("fetch_plan failed: {e}"); Error::internal(e) })?
        .into_iter()
        .next()
        .ok_or_else(|| { tracing::error!("plan not found: {id}"); Error::Internal("plan not found: {id}".into()) })
}
...
fn fetch_teacher(conn: &mut Conn, school: &str, user: &str) -> Result<TeacherRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("teacher not found: {school}|{user}".into())
    })
}
...
fn fetch_staff(conn: &mut Conn, school: &str, user: &str) -> Result<StaffRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("staff not found: {school}|{user}".into())
    })
}
...
fn fetch_guardian(conn: &mut Conn, school: &str, user: &str, student: i32) -> Result<GuardianRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("guardian not found: {school}|{user}|{student}".into())
    })
}
...
fn fetch_student(conn: &mut Conn, school: &str, adm: i32) -> Result<StudentRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("student not found: {school}|{adm}".into())
    })
}
...
fn fetch_enrollment(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("enrollment not found: {school}|{year}|{term}|{grade}|{stream}|{student}".into())
    })
}
...
fn fetch_department(conn: &mut Conn, school: &str, name: &str) -> Result<DepartmentRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("department not found: {school}|{name}".into())
    })
}
...
fn fetch_term(conn: &mut Conn, school: &str, year: i32, term: i16) -> Result<TermRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("term not found: {school}|{year}|{term}".into())
    })
}
...
fn fetch_class_teacher(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("class_teacher not found: {school}|{year}|{term}|{grade}|{stream}|{teacher}".into())
    })
}
...
fn fetch_subject_teacher(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("subject_teacher not found: {school}|{year}|{term}|{grade}|{stream}|{subject}".into())
    })
}
...
fn fetch_timetable(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("timetable not found: {school}|{year}|{term}|{grade}|{stream}|{subject}|{day}|{start}".into())
    })
}
...
fn fetch_attendance(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("record not found".into())
    })
}
...
fn fetch_exam(conn: &mut Conn, id: &str) -> Result<ExamRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("exam not found: {id}".into())
    })
}
...
fn fetch_paper(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("paper not found: {school}|{exam}|{subject}|{paper:?}|{grade}|{stream:?}".into())
    })
}
...
fn fetch_mastery(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("mastery not found: {school}|{student}|{subject}|{topic}".into())
    })
}
...
fn fetch_subject_catalog(conn: &mut Conn, id: i32) -> Result<SubjectCatalogRow> {
    sql_query("SELECT id, name, curriculum, created, updated FROM subjects WHERE id = ?")
        ...
        .ok_or_else(|| {
            ...
            Error::Internal("subject not found: {id}".into())
        })
}
...
fn fetch_topic(conn: &mut Conn, id: i32) -> Result<TopicRow> {
    sql_query("SELECT id, subject, grade, name, created, updated FROM topics WHERE id = ?")
        ...
        .ok_or_else(|| {
            ...
            Error::Internal("topic not found: {id}".into())
        })
}
...
fn fetch_stream(conn: &mut Conn, school: &str, grade: i16, stream: i16) -> Result<StreamRow> {
    ...
    .bind::<diesel::sql_types::SmallInt, _>(stream)
    .load::<StreamRow>(conn)
    .map_err(|e| { tracing::error!("fetch_stream failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("stream not found: {school}|{grade}|{stream}"); Error::Internal("stream not found: {school}|{grade}|{stream}".into()) })
}
...
fn fetch_mpesa(conn: &mut Conn, school: &str) -> Result<MpesaRow> {
    ...
    .bind::<Text, _>(school)
    .load::<MpesaRow>(conn)
    .map_err(|e| { tracing::error!("fetch_mpesa failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("mpesa not found: {school}"); Error::Internal("mpesa not found: {school}".into()) })
}
...
fn fetch_fee(conn: &mut Conn, id: &str) -> Result<FeeRow> {
    ...
    .bind::<Text, _>(id)
    .load::<FeeRow>(conn)
    .map_err(|e| { tracing::error!("fetch_fee failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("fee not found: {id}"); Error::Internal("fee not found: {id}".into()) })
}
...
fn fetch_invoice(conn: &mut Conn, id: &str) -> Result<InvoiceRow> {
    ...
    .map_err(|e| { tracing::error!("fetch_invoice failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("invoice not found: {id}"); Error::Internal("invoice not found: {id}".into()) })
}
...
fn fetch_payment(conn: &mut Conn, id: &str) -> Result<PaymentRow> {
    ...
    .map_err(|e| { tracing::error!("fetch_payment failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("payment not found: {id}"); Error::Internal("payment not found: {id}".into()) })
}
...
fn fetch_announcement(conn: &mut Conn, id: &str) -> Result<AnnouncementRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("announcement not found: {id}".into())
    })
}
...
fn fetch_role(conn: &mut Conn, id: &str) -> Result<RoleRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("role not found: {id}".into())
    })
}
...
fn fetch_scope(conn: &mut Conn, school: Option<&str>, user: &str, role: &str) -> Result<ScopeRow> {
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("scope not found: {user}/{role}".into())
    })
}
...
fn fetch_ai_usage(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("ai_usage not found: {school}/{student}/{year}/{term}".into())
    })
}
...
fn fetch_subscription(
    ...
    .map_err(|e| { tracing::error!("fetch_subscription failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("subscription not found"); Error::Internal("subscription not found".into()) })
}
...
fn fetch_discount(
    ...
    .ok_or_else(|| {
        ...
        Error::Internal("discount not found".into())
    })
}
...
fn handle_create_subject(conn: &mut Conn, payload: &[u8]) -> Result<ActionResult> {
    ...
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("subject not found after insert"); Error::Internal("subject not found after insert".into()) })?;
...
fn handle_create_topic(conn: &mut Conn, payload: &[u8]) -> Result<ActionResult> {
    ...
    .bind::<diesel::sql_types::Text, _>(&p.name)
    .load::<TopicRow>(conn)
    .map_err(|e| { tracing::error!("fetch after insert_topic failed: {e}"); Error::internal(e) })?
    .into_iter()
    .next()
    .ok_or_else(|| { tracing::error!("topic not found after insert"); Error::Internal("topic not found after insert".into()) })?;
